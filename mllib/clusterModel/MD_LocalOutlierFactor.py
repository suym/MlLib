...
def main_model(dir_of_dict):
    ...
    model_name = 'MD_LocalOutlierFactor'
    ...
    print'----------------------------------------------'
    best_neighbor,best_contamination = mlp.GS_LocalOutlierFactor_parameter(X)
    clst_labels,scores_pred = mlp.Model_LocalOutlierFactor(X,best_neighbor,best_contamination)
    #添加异常原因，并给出异常值和基准值
